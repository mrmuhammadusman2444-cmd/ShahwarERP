...
const AddSupplier = () => {
  ...
  async function handleSupplier() {
    try {
      ...
      formData.append('supplierName', addSupplier.supplierName || '')
      formData.append('email', addSupplier.email || '')
      formData.append('phoneNo', addSupplier.phoneNo || '')
      formData.append('address', addSupplier.address || '')
      formData.append('supplierDetails', addSupplier.supplierDetails || '')
      formData.append('supplierCredits', addSupplier.supplierCredits || '')
      formData.append('previousCreditsBalance', addSupplier.previouseCreditsBalance || '')
      if (picture) formData.append('picture', picture)

      let res = await axios.post('http://localhost:3000/new/supplier', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      console.log(res.data)
    } catch (err) {
      console.log("SUPPLIER SAVE FAILED:", err.response?.data || err.message)
    }
  }
        ...
        <div className="w-full lg:w-52 shrink-0 flex flex-col gap-4 sticky top-4">
          ...
          <div className="bg-white border border-emerald-100 rounded-2xl shadow-sm p-4 flex flex-col items-center gap-3">
            ...
            <div className="w-full border-t border-emerald-50 pt-3 flex flex-col gap-2">
              {[
                { label: "Phone No", value: addSupplier.phoneNo || "Not set" },
                { label: "Email", value: addSupplier.email || "Not set" },
                { label: "Address", value: addSupplier.address || "Not set" },
              ].map((item) => (
