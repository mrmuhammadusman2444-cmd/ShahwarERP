    ...
    try {
      ...
      Object.keys(grouped).forEach((category) => {
        ...
        grouped[category].forEach((item) => {
          ...
          rows.push([counter++, item.name, item.carton || 0, item.storeLimit || item.cartonSize || "-", `Rs. ${cartonRate.toLocaleString()}`, `Rs. ${Number(item.total || 0).toLocaleString()}`])
        })
      })
      autoTable(doc, {
        startY: 62,
        ...
        columnStyles: { 0: { cellWidth: 12, halign: "left" }, 1: { halign: "left" }, 2: { halign: "center" }, 3: { halign: "center" }, 4: { halign: "center" }, 5: { halign: "center" } },
      })
    ...
    <div className="p-3 sm:p-4 md:p-5">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-linear-to-br from-emerald-500 to-emerald-600 flex items-center justify-center shadow-md shadow-emerald-200 shrink-0">
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <div>
            <h1 className="text-gray-800 text-base sm:text-lg font-bold">Customer Ledger</h1>
      ...
      <div className="bg-white border border-slate-200/70 rounded-2xl shadow-sm p-3 sm:p-4 mb-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-end gap-3 flex-wrap">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            <div className="flex-1 sm:w-auto">
              ...
              <input value={fromDate} onChange={(e) => setFromDate(e.target.value)} type="date"
                className="w-full bg-emerald-50/50 border border-emerald-100 focus:border-emerald-400 focus:bg-white rounded-xl px-3 py-2.5 text-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-100 transition-all" />
            </div>
            <div className="flex-1 sm:w-auto">
          ...
          <div className="grid grid-cols-2 gap-2 w-full sm:w-auto pb-0.5">
            <button onClick={handleSearch} className="flex items-center justify-center cursor-pointer gap-2 px-4 py-2.5 bg-linear-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white text-sm font-semibold rounded-xl shadow-md shadow-emerald-200 transition-all hover:-translate-y-0.5">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
              Search
            </button>
            <button onClick={handlePrint} className="flex items-center justify-center cursor-pointer gap-2 px-4 py-2.5 bg-white border border-emerald-200 hover:bg-emerald-50 text-emerald-600 text-sm font-semibold rounded-xl transition-all hover:-translate-y-0.5">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white border border-slate-200/70 rounded-2xl shadow-sm overflow-hidden">
        ...
        {/* Card Header */}
        <div className="px-3 sm:px-5 py-3 border-b border-slate-100 bg-linear-to-r from-emerald-50/50 to-white flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 sm:gap-2.5 flex-wrap">
            ...
            <h2 className="text-gray-800 text-xs sm:text-sm font-bold truncate max-w-28 sm:max-w-none">{selectedCustomer || "No customer"}</h2>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-linear-to-b from-emerald-500 to-emerald-700 text-white sticky top-0 z-10">
                <th className="text-left text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap">Date</th>
                <th className="text-left text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3">Description</th>
                <th className="text-left text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap">Invoice ID</th>
                <th className="text-left text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap hidden md:table-cell"> Deposit ID</th>
                <th className="text-right text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap">Debit</th>
                <th className="text-right text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap">Credit</th>
                <th className="text-right text-[12px] font-semibold tracking-wider px-2 sm:px-4 py-3 whitespace-nowrap">Balance</th>
              </tr>
            </thead>

            <tbody>
              <tr className="bg-emerald-50 border-b border-emerald-100">
                <td className="px-2 sm:px-4 py-3 text-left" colSpan={6}>
                ...
                (() => {
                  ...
                  return filteredEntries.map((entry, idx) => (
                    <tr key={idx} className="group relative border-b border-gray-50 hover:bg-emerald-50/50 transition-colors">
                        ...
                        {entry.invoiceId ? (
                          <button onClick={() => handleDownloadInvoice(entry.invoiceId)}
                            ...
                            <span className="group-hover/desc:underline">
                              {Array.isArray(entry.description) ? entry.description.map((line, i) => <p key={i}>{line}</p>) : entry.description}
                            </span>
                          </button>
                        ) : (
                      ...
                      <td className="px-2 sm:px-4 py-3.5 text-left whitespace-nowrap">
                        {entry.invoiceId ? (
                      ...
                      <td className="px-2 sm:px-4 py-3.5 text-left text-gray-400 text-[11px] font-mono whitespace-nowrap hidden md:table-cell">
                        {entry.depositId || "—"}
                      </td>
