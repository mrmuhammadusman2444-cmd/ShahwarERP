          ...
          <div className="relative w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea rows={2} value={editTx.description || ""} onChange={(e) => setEditTx({ ...editTx, description: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2.5 focus-within:border-emerald-400 focus-within:bg-white transition-all">
                    ...
                    <input type="number" value={editTx.debit || ""} onChange={(e) => setEditTx({ ...editTx, debit: e.target.value })} placeholder="0"
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/60 px-3 py-2.5 focus-within:border-emerald-400 focus-within:bg-white transition-all">
                    <span className="text-xs font-bold text-slate-400">Rs</span>
                    <input type="number" value={editTx.credit || ""} onChange={(e) => setEditTx({ ...editTx, credit: e.target.value })} placeholder="0"
                      className="w-full bg-transparent text-sm text-slate-800 focus:outline-none" />
                  </div>
                </div>
              </div>
        ...
        {tabs.map((t) => {
          ...
          return (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`group flex items-center gap-2.5 rounded-2xl px-5 py-3 text-sm font-bold transition-all cursor-pointer ${active
      ...
      <div className="rounded-3xl border border-slate-200/70 bg-white shadow-xl shadow-slate-200/50 overflow-hidden">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold text-slate-800">{activeLabel} Receipt Report</h2>
            <span className="rounded-full bg-emerald-50 px-2.5 py-0.5 text-[10px] font-bold text-emerald-600 ring-1 ring-emerald-100">{activeTab === 'bank' ? bankFiltered.length : 0} records</span>          </div>
          <div className="flex items-center gap-2 rounded-full bg-slate-50 border border-slate-200 px-4 py-2 focus-within:border-emerald-400 focus-within:bg-white transition-all">
            <Search className="h-3.5 w-3.5 text-emerald-400 shrink-0" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} type="text" placeholder={`Search ${activeLabel.toLowerCase()}...`}
                    ...
                    <tr key={t._id} className={`transition-colors hover:bg-emerald-50/50 ${i % 2 === 1 ? "bg-slate-50/40" : "bg-white"}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center">
                          <button onClick={() => setEditTx(t)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-sky-50 text-sky-500 hover:bg-sky-500 hover:text-white transition-colors cursor-pointer">
                            <Pencil size={14} />
                          </button>
                        </div>
                      </td>
