...
const PurchaseApproval = () => {
    const purchases = [];
    const totalAmount = purchases.reduce((sum, inv) => sum + (Number(inv.amount) || 0), 0)
            ...
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-400 text-xs">Review and approve pending Purchase Approvals</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                ...
                <div className="bg-white border border-amber-100 rounded-xl shadow-sm p-3.5 flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-gray-800 text-base font-bold leading-tight">{PurchaseApproval.length}</p>
                    </div>
                </div>
            ...
            <div className="bg-white border border-blue-100 rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <table className="w-full text-sm border-collapse">
                        <thead>
                            <tr className="bg-blue-600 text-white">
                                ...
                                <th className="w-[14%] text-left text-xs font-semibold px-4 py-3">Description</th>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2.5">
                                                <div className="w-7 h-7 rounded-full bg-blue-100 text-blue-600 text-xs font-bold flex items-center justify-center shrink-0">
                                                    {PurchaseApproval.customerName ? PurchaseApproval.customerName.charAt(0).toUpperCase() : "?"}
                                                </div>
                                                <div className="min-w-0">
                                                    <p className="text-gray-700 text-xs font-medium truncate">{PurchaseApproval.customerName}</p>
                                                    {PurchaseApproval.customerPhone && (
                                                        <p className="text-gray-400 text-xs">{PurchaseApproval.customerPhone}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </td>

                                        <td className="px-4 py-3 text-gray-500 text-xs">{v.description}</td>

                                        <td className="px-4 py-3 text-right text-gray-800 text-xs font-bold whitespace-nowrap">
                                            Rs {PurchaseApproval.amount}
                                        </td>

                                        <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{PurchaseApproval.user}</td>
                ...
                {PurchaseApproval.length > 0 && (
                    <div className="px-5 py-3 border-t border-blue-50 bg-blue-50/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                        <p className="text-xs text-gray-400">Total InvoiceApproval: <span className="font-semibold text-gray-600">{purchases.length}</span></p>
