...
const SelectProducts = ({ value, onChange }) => {
    ...
    const selected = products.find((p) => p.itemName === value);
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/find/items')
            .then(res => setProducts(res.data.filter((i) => i.category === "RM" || i.category === "PM")))
            .catch(err => console.log("ITEMS FAILED:", err.message))
    }, [])
            ...
            >
                <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="truncate text-[12px] font-medium">
                        {selected ? selected.itemName : "Select product"}
                    </span>
                </div>
                <ChevronDown
