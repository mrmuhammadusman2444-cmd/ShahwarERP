...
import { ClipboardList, User, Calendar, Search, Printer, ChevronDown, Home, ChevronRight, Building2 } from 'lucide-react'

const SupplierReciept = () => {
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-emerald-50/40 p-4 md:p-6">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-xl font-bold leading-tight text-slate-800">Supplier Receipt</h1>
            <p className="text-[11px] font-medium tracking-wide text-slate-400">Supplier Receipt</p>
          </div>
        </div>
        ...
        <div className="p-5 md:p-6">
          <div className="flex flex-col lg:flex-row lg:items-end gap-3">
            ...
            <div className="flex-1 min-w-48">
              <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-widest text-slate-500">Supplier</label>
              <div className="relative">
                <div
                  onClick={() => setSupplierOpen((o) => !o)}
                  className={`flex cursor-pointer items-center gap-2 rounded-xl border bg-slate-50/60 px-3 py-2.5 transition-all ${supplierOpen ? 'border-emerald-400 bg-white ring-4 ring-emerald-50' : 'border-slate-200 hover:border-emerald-300'}`}>
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-emerald-100 text-emerald-600"><Building2 className="h-3 w-3" /></span>
                  <span className="flex-1 truncate text-xs font-medium text-slate-400">Select option</span>
                  <ChevronDown className={`h-3.5 w-3.5 shrink-0 text-emerald-400 transition-transform duration-200 ${supplierOpen ? 'rotate-180' : ''}`} />
                </div>
                ...
                {supplierOpen && (
                  <div className="absolute left-0 right-0 top-full z-50 mt-1.5 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl shadow-slate-300/40">
                    <div className="flex items-center gap-2 border-b border-slate-100 px-3 py-2">
                      ...
                      <input autoFocus placeholder="Search supplier..." className="w-full bg-transparent text-xs text-slate-700 placeholder-slate-400 focus:outline-none" />
                    </div>
                    <div className="max-h-56 overflow-y-auto py-1">
                      <p className="px-4 py-6 text-center text-xs text-slate-400">No suppliers</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-linear-to-r from-emerald-600 to-emerald-700">
                    ...
                    <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wide text-white">Supplier</th>
                    <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wide text-white">Detail</th>
                  ...
                  {records.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-20 text-center">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-[11px] text-slate-400">Select a supplier and search</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    records.map((r, i) => (
                      <tr key={i} className={`transition-colors hover:bg-emerald-50/50 ${i % 2 === 1 ? "bg-slate-50/40" : "bg-white"}`}>
                        ...
                        <td className="px-4 py-2.5 text-xs font-medium text-slate-800">{r.supplier}</td>
                        <td className="px-4 py-2.5 text-xs text-slate-600">{r.detail}</td>
...
export default SupplierReciept
